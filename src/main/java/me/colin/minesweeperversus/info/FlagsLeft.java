package me.colin.minesweeperversus.info;

import me.colin.minesweeperversus.GameBoardPanel;
import me.colin.minesweeperversus.fonts.Roboto;

import javax.swing.*;
import java.awt.*;
import java.net.MalformedURLException;
import java.net.URL;

public class FlagsLeft extends JLabel {

    private static JLabel jLabel;

    public static int left;

    public FlagsLeft() {
        jLabel = this;
        try {
            ImageIcon icon = new ImageIcon(new URL("https://www.google.com/logos/fnbx/minesweeper/flag_icon.png"));
            Image image = icon.getImage(); // transform it
            ImageIcon icon2 = new ImageIcon(image.getScaledInstance(50, 50,  Image.SCALE_SMOOTH)); // scale it the smooth way
            super.setIcon(icon2);
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }

        super.setHorizontalTextPosition(RIGHT);
        super.setFont(Roboto.REGULAR.deriveFont(30f));
        super.setForeground(Color.WHITE);

        left = GameBoardPanel.numMines;
        update();
    }

    public static void update() {
        jLabel.setText(String.valueOf(left));
    }

}